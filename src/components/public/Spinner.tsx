...
const Spinner: React.FC<SpinnerProps> = ({ animateOut }) => {
  return (
    <div
      id="preloader"
      className={`fixed top-0 left-0 w-full h-full z-[9999] bg-white dark:bg-[#141427] ${
        animateOut ? "fade-out" : ""
      }`}
    >
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        {/* Outer Circle Border */}
        <motion.div
          className="relative inset-0 w-[85px] h-[85px] rounded-full border-[3px] border-gray-300 dark:border-black"
          animate={{ rotate: 900 }}
          transition={{
            repeat: Infinity,
            duration: 1.1,
