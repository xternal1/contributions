...
const Exam = () => {
    ...
    }, [MySwal, navigate, slug, resetStore]);

    // Watch for time up
    useEffect(() => {
        if (timeLeft === 0 && pretest) {
            handleTimeUpAlert();
        }
    }, [timeLeft, pretest, handleTimeUpAlert]);

    // Handle submit
    const handlePageSubmit = useCallback(async () => {
        if (!pretest?.user_quiz?.id) return;
        ...
        try {
            ...
            if (result.success) {
                navigate(`/course/pre-tes/exam/results/${pretest.user_quiz.id}`, {
                    state: { testResult: result.testResult },
                });
            }
        } catch (err) {
    ...
    }, [pretest, submitAnswers, MySwal, navigate]);
            ...
            <div className="2xl:max-w-6xl xl:max-w-6xl lg:max-w-5xl md:max-w-2xl sm:max-w-xl max-w-sm mx-auto mt-8">
                ...
                <div className="relative min-h-5 bg-gradient-to-br from-purple-500 to-purple-700 rounded-xl shadow p-6 mb-6 flex justify-between">
                    ...
                    <div className="text-center px-5">
                        <p className="text-sm lg:text-lg md:text-md sm:text-sm text-purple-700 bg-white py-2 px-4 rounded-lg font-semibold">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 pb-20">
                                    ...
                                    (key) => (
                                        ...
                                        >
                                            <input
                                                ...
                                                checked={answers[currentPage] === key}
                    ...
                    <div className="bg-white rounded-lg shadow p-8 flex flex-col justify-between self-start dark:bg-[#0D0D1A] dark:border-2 dark:border-white">
                        <div>
                            <h3 className="text-lg text-start font-semibold text-gray-800 mb-3 dark:text-white">
                                Soal Ujian
                            </h3>
                            ...
                            <div className="grid grid-cols-7 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-3 md:gap-4 lg:gap-5 mb-6">
                                {Array.from({ length: totalQuestions }).map((_, i) => {
                                    ...
                                    const isAnswered = answers[pageNumber] !== undefined;
