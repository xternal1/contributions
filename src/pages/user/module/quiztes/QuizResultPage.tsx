...
const QuizResultPage: React.FC = () => {
  ...
  if (resultLoading) return <QuizResultSkeleton />;

  if (resultError) {
    return (
      <div className="min-h-screen flex items-center justify-center dark:bg-[#141427]">
        <p className="text-xl font-semibold text-red-500">{resultError}</p>
      </div>
    );
  }
      ...
      <div className="2xl:max-w-6xl xl:max-w-5xl lg:max-w-4xl md:max-w-2xl sm:max-w-xl max-w-md mx-auto mt-8">
        ...
        <div className="relative min-h-37 bg-gradient-to-br from-purple-500 to-purple-700 rounded-xl shadow p-6 mb-6 flex flex-col md:flex-row items-center md:items-center justify-between">
          <div className="text-left px-5 mb-4 md:mb-0 md:flex-1">
            ...
            <h2 className="text-2xl font-bold text-white">{headerMessage || "Selamat anda telah menyelesaikan test"}</h2>
            <p className="text-white mt-1 sm:text-base md:text-base">Hasil test anda akan tampilan dibawah ini</p>
          </div>
          <div className="flex justify-center md:justify-end w-full md:w-auto">
            <img src={imgBook} alt="Ilustrasi Ujian" className="w-80 sm:w-80 md:w-60 mx-8 mt-6 md:mt-0" />
          </div>
        </div>
        ...
        <div className="2xl:max-w-6xl xl:max-w-5xl lg:max-w-5xl md:max-w-2xl sm:max-w-xl max-w-md mx-auto mt-8 grid grid-cols-1 2xl:grid-cols-[0.7fr_2fr] xl:grid-cols-[0.7fr_2fr] lg:grid-cols-3 items-start gap-6">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow p-6 dark:bg-[#0D0D1A] dark:border-2 dark:border-white transition-colors duration-500">
              ...
              <div className="mb-4">
                {result.status === "Lulus" ? (
                  <button className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-2 rounded-lg transition-colors duration-200">Selamat Anda Lulus</button>
                ) : (
                  <button className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-2 rounded-lg transition-colors duration-200">Maaf Anda Tidak Lulus</button>
                )}
              </div>
            </div>
                ...
                <div key={q.question + idx} className="bg-white rounded-lg shadow p-7 dark:bg-[#0D0D1A] dark:border-2 dark:border-white transition-colors duration-500">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4 gap-3">
                    <div className="text-start flex-1 min-w-0">
                      <h3 className="font-semibold flex items-start gap-2">
                        ...
                        <span className="break-words overflow-hidden text-justify leading-relaxed" dangerouslySetInnerHTML={{ __html: q.question }} />
                      </h3>
                    </div>

                    <span className={`inline-flex items-center gap-2 rounded-lg py-1 px-3 text-sm font-semibold flex-shrink-0 w-fit sm:w-auto ${isCorrect ? "bg-purple-100 text-purple-600 dark:bg-purple-950" : "bg-red-100 text-red-600 dark:bg-red-950"}`}>
                      {isCorrect ? (<><Check className="w-4 h-4 flex-shrink-0" /><span>Benar</span></>) : (<><X className="w-4 h-4 flex-shrink-0" /><span>Salah</span></>)}
                    </span>
                  </div>
                  ...
                  <div className="mt-4 space-y-3 text-sm">
                    {["option_a", "option_b", "option_c", "option_d", "option_e"].map((opt) => {
                      ...
                      return (
                        <label key={opt} className={`flex items-start space-x-3 p-3 rounded-lg transition-colors duration-200 border ${isUserAnswer ? (isCorrect ? "bg-purple-50 dark:bg-purple-900/30 border-purple-300 dark:border-purple-700 text-purple-800 dark:text-purple-200 font-semibold" : "bg-red-50 dark:bg-red-900/30 border-red-300 dark:border-red-700 text-red-800 dark:text-red-200 font-semibold") : "text-gray-700 dark:text-white border-gray-200 dark:border-gray-600"}`}>
                          <input type="radio" disabled checked={isUserAnswer} className="relative appearance-none w-5 h-5 rounded-full mt-0.5 flex-shrink-0 border-2 border-purple-600 cursor-pointer" />
                          <span className="break-words overflow-hidden text-justify leading-relaxed flex-1" dangerouslySetInnerHTML={{ __html: optionText }} />
                        </label>
                      );
                    })}
                  </div>
